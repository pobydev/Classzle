...
export interface Student {
  ...
  behavior_note?: string;        // 비고 (툴팁용)
  is_pre_transfer?: boolean;     // 전출 예정 여부

  // 그룹 및 관계
  group_ids: string[];           // 소속된 CustomGroup ID 목록
  avoid_ids: string[];           // 피해야 할 학생 ID (상극)
  avoid_memos?: Record<string, string>; // 분리 배정 사유 (Key: 상대방 ID, Value: 사유)
...
export interface AppSettings {
  ...
  scoreTolerance: number;        // 성적 유사군 허용 범위 (±)
}
...
export interface ClassStats {
  ...
  // 레거시 호환 (필요 시 유지, 또는 로직에서 합산하여 사용)
  scorePlus1: number;
  scorePlus2: number;
  scorePlus3: number;
